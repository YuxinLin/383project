 ...
 *shortestJobFirst return type -> char* -> void
 *free -> time_chart
 *curr_quantum >= MAX_QUANTA - 1 -> curr_quantum >= MAX_QUANTA
 */
...
void shortestJobFirst(struct process* jobs_list)
{
	...
	while(!done) {
		...
		if(curr_quantum >= MAX_QUANTA || num_finished_jobs == NUM_PROCESS) { //MAX_QUANTA - 1
	...
	//don't forget to free the memory space for malloc
	free(jobs_queue);
	free(time_chart);
