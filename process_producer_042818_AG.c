...
//PrintProcessList for non preemptive
void PrintProcessList(struct process process_list[]){
	...
	for(int i = 0; i < NUM_PROCESS; i++){
		printf(" %3c | %7.1f | %5d | %8d | %7.1f | %8.1f | %5.1f | %7.1f \n", 
            ...
            process_list[i].actual_start_time,      //Start
            process_list[i].priority,               //Priority
			process_list[i].service_time,           //Service
            process_list[i].actual_start_time - process_list[i].arrival_time,       //Response
			process_list[i].actual_start_time + process_list[i].service_time,       //End
			process_list[i].actual_start_time + process_list[i].service_time - process_list[i].arrival_time); //Turnaround
        
    }
}
//PrintProcessList for preemptive
void PrintProcessListPre(struct process process_list[]){
    printf(" PID | Arrival | Start | Priority | Service | Response |  End  | Turnaround\n");
    for(int i = 0; i < NUM_PROCESS; i++){
        printf(" %3c | %7.1f | %5d | %8d | %7.1f | %8.1f | %5d| %7.1f \n",
               process_list[i].name,
               process_list[i].arrival_time,
               process_list[i].actual_start_time,
               process_list[i].priority,
               process_list[i].service_time,
               process_list[i].actual_start_time - process_list[i].arrival_time,
               process_list[i].end_time,
               process_list[i].end_time - process_list[i].arrival_time);
    }
}
